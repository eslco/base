...
# cleanup() {
#     echo
#     echo -e "${yellow}科技lion工具箱已被终止!${white}"
#     send_stats "非法退出脚本"
#     exit 0
# }
...
#     status_message="${gray}采集已关闭${white}"
# else
...
UserLicenseAgreement() {
    clear
    echo esimpo_jilion.sh @254c_mod
...
while true; do
    ...
    case $sub_choice in
        ...
        8)
            send_stats "删除所有容器"
            read -p "$(echo -e "${red}注意: ${white}确定删除所有容器吗？(Y/N): ")" choice
            case "$choice" in
              [Yy])
...
while true; do
    ...
    case $sub_choice in
        ...
        4)
            send_stats "删除所有镜像"
            read -p "$(echo -e "${red}注意: ${white}确定删除所有镜像吗？(Y/N): ")" choice
            case "$choice" in
              [Yy])
...
if [ -e /home/web/conf.d/$yuming.conf ]; then
  send_stats "域名重复使用"
  echo -e "${yellow}提示: ${white}当前 ${yuming} 域名已被使用，请前往31站点管理，删除站点，再部署 ${webname} ！"
  break_end
  linux_ldnmp
fi
...
server_reboot() {
    ...
    read -p "$(echo -e "${yellow}提示: ${white}现在重启服务器吗？(Y/N): ")" rboot
    case "$rboot" in
      [Yy])
        echo "已重启"
        reboot
...
ldnmp_install_status_two() {
   ...
   if docker inspect "php" &>/dev/null; then
    ...
    echo -e "${yellow}提示: ${white}LDNMP环境已安装。无法还原LDNMP环境，请先卸载现有环境再次尝试还原。"
    break_end
    linux_ldnmp
   else
    :
...
ldnmp_install_status() {
   ...
   if docker inspect "php" &>/dev/null; then
    ...
    echo -e "${yellow}提示: ${white}LDNMP环境未安装，请先安装LDNMP环境，再部署网站"
    break_end
    linux_ldnmp
...
nginx_install_status() {
   ...
   if docker inspect "nginx" &>/dev/null; then
    ...
    echo -e "${yellow}提示: ${white}nginx未安装，请先安装nginx环境，再部署网站"
    break_end
    linux_ldnmp
...
dd_xitong() {
        ...
        dd_xitong_4() {
          ...
          while true; do
            ...
            echo -e "${gray}感谢MollyLau大佬和bin456789大佬的脚本支持！${white} "
            echo "------------------------"
...
clamav() {
          ...
          while true; do
                ...
                echo -e "${yellow}提示: ${white} 目前该工具仅支持x86架构系统，不支持ARM架构！"
                echo "------------------------"
...
linux_tools() {
  ...
  while true; do
      ...
      echo -e "${cyan}13.  ${white}gdu 磁盘占用查看工具              ${cyan}14.  ${white}fzf 全局搜索工具"
      echo -e "${cyan}15.  ${white}vim 文本编辑器                    ${cyan}16.  ${white}nano 文本编辑器 ${yellow}★${white}"
      echo -e "${cyan}13.  ${white}git 版本控制系统"
      echo -e "${cyan}------------------------"
              ...
              while true; do
                  ...
                  case $sub_choice in
                      ...
                      2)
                       ...
                       3)
                          send_stats "删除所有卷"
                          read -p "$(echo -e "${red}注意: ${white}确定删除所有未使用的卷吗？(Y/N): ")" choice
                          case "$choice" in
                            [Yy])
...
linux_ldnmp() {
  ...
  while true; do
    ...
    while true; do
        ...
        echo "站点目录"
        echo "------------------------"
        echo -e "数据 ${gray}/home/web/html${white}     证书 ${gray}/home/web/certs${white}     配置 ${gray}/home/web/conf.d${white}"
        echo "------------------------"
        echo ""
        echo "操作"
        echo "------------------------"
...
linux_panel() {
    ...
    while true; do
      ...
      echo -e "${cyan}39.  ${white}Bililive直播录制工具                ${cyan}40.  ${white}远程Windows11"
      echo -e "${cyan}------------------------"
...
linux_Settings() {
    ...
    while true; do
      ...
      echo -e "${cyan}29.  ${white}病毒扫描工具 ${yellow}★${white}"
      echo -e "${cyan}------------------------"
      echo -e "${cyan}31.  ${white}留言板                             ${cyan}66.  ${white}一条龙系统调优 ${yellow}★${white}"
      echo -e "${cyan}------------------------"
...
EOF
          ...
          25)
              ...
              echo "到达阈值后会向用户发预警消息"
              echo -e "${gray}-关于流量，重启服务器将重新计算-${white}"
              read -p "确定继续吗？(Y/N): " choice
              ...
              case "$choice" in
                [Yy])
                  ...
                  clear
                  echo "TG-bot预警系统已启动"
                  echo -e "${gray}你还可以将root目录中的TG-check-notify.sh预警文件放到其他机器上直接使用！${white}"
                  ;;
                [Nn])
                  echo "已取消"
                  ;;
